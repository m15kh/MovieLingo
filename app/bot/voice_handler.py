...
from app.database.models import Video, VideoAttempt
from app.services import whisper_service, challenge_service
...
class VoiceHandler:
    ...
    async def handle_voice(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                if is_match:
                    # Success!
                    await self.handle_successful_voice(update, context, db, video_attempt)
                else:
                    # Failed attempt
    ...
    async def show_subtitle(self, update: Update, context: ContextTypes.DEFAULT_TYPE, db, video_attempt: VideoAttempt):
    ...
    async def handle_successful_voice(self, update: Update, context: ContextTypes.DEFAULT_TYPE, db, video_attempt: VideoAttempt):
        """Handle successful voice recognition"""
        await update.message.reply_text(
            f"✅ Perfect! That's correct!\n\n"
    ...
    async def handle_failed_voice(self, update: Update, context: ContextTypes.DEFAULT_TYPE, db, video_attempt: VideoAttempt):
        """Handle failed voice attempts"""
        video = await db.get(Video, video_attempt.video_id)
        
        await update.message.reply_text(
            f"😔 You couldn't say it today.\n\n"
    ...
    async def send_question(self, update: Update, context: ContextTypes.DEFAULT_TYPE, db, video_attempt: VideoAttempt):
